...
[![Docs](https://img.shields.io/badge/docs-website-blue.svg)](https://multiagent-protocol.github.io/)
...
- **Pre-merge gate (L1)**: 5 conditions a PR must satisfy before it can merge (label, CI green, owner approval or auto-approval per classifier, base up-to-date, identity trailers present).
- **Post-merge re-validation (L2)**: same checks rerun on the merged commit; auto-revert if they fail post-merge but not for infrastructure-only failures.
- **Race-guard (L3)**: re-check the PR's base against `origin/main` HEAD just before merge; auto-rebase + re-CI on drift.
- **Identity gate (L4)**: validate every commit's `Agent-Tool`, `Agent-Model`, `Agent-Session`, `Agent-Machine`, `Task-Ref` trailers against your registry.
- **Break-glass auditor (L5)**: scan `main` for `[break-glass-*]` commits and require an ADR within 24 hours.
...
1. Open [https://multiagent-protocol.github.io/wizard/](https://multiagent-protocol.github.io/wizard/) in your browser.
2. Fill in: your GitHub login, the repos you want supervised, your preferred runner tier, and which built-in skills to enable.
3. The wizard generates 4 YAML config files + a 1-click GitHub App registration URL.
...
- **v1.0.0**: first public release (target: when docs + web wizard + example repos all complete).
...
- [`docs/guide/break-glass.md`](docs/guide/break-glass.md) — when and how to override the bot
